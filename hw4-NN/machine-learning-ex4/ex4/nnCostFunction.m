...
ry = repmat([1:num_labels], m, 1) == repmat(y, 1, num_labels);

cost = ry.*log(a3) + (1-ry).*log(1-a3);
J = -sum(sum(cost,2))/m;

regJ = lambda/(2*m)*(sum(sum((Theta1(:,2:end)).^2,2))+sum(sum((Theta2(:,2:end)).^2,2)));
J = J+regJ;

% implement the backpropogation algorithm to compute the gradients
delta3 = a3-ry;
